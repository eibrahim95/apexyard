...
| Zed (native Zed agent) | Neither — `AGENTS.md` is the entry point and `CLAUDE.md` is not auto-loaded. Read **"Zed overlay (native Zed agent)"** below, then `CLAUDE.md`, then the rules you need on demand |
...
### Zed overlay (native Zed agent)

Zed has two agent paths, and they enforce differently:

- **Claude Agent over ACP** — Claude Code runs as an External Agent in Zed's agent panel. It reads `CLAUDE.md` and `.claude/` itself, so the full enforcement layer applies. Prefer this path when you can choose.
- **Native Zed agent** — Zed reads `AGENTS.md` first and loads skills from `.agents/skills/` (generated by `bin/sync-zed-adapter.sh`). Read `CLAUDE.md` now, then load the rule you need from `.claude/rules/` on demand. The rules are the same rules.

Under the native agent, run `bin/install-zed-adapter.sh` once per machine. It installs the tool-permission rules Zed can enforce: it **denies** `git add -A`, `git add .`, `gh pr merge` and `gh api .../merge`, and it **asks for confirmation** on `gh pr create` and `gh issue create`.

Every other gate is **advisory** under the native agent. Follow each one yourself:

- ticket-first — no active-ticket marker is enforced
- the merge gate — the Rex marker and the explicit per-PR nod
- the red-CI merge block
- the reviewer read-only rule — a `spawn_agent` subagent inherits your tools, so instruct it to review only
- the secrets and private-ref scans
- AgDR-required for architecture changes
- branch-name, commit-format, PR-title and PR-body validation
- the migration, architecture-review and design gates

Merges go through `tracker_pr_merge`, whose wrapper runs the merge gates (AgDR-0162). Do not run `gh pr merge` or `gh api .../merge` directly. Per-gate status table and the capability manifest: `docs/harnesses/zed.md`.
