...
## No tool-attribution banners in product artifacts (advisory)

Attribution cuts both ways. On **commits and PRs**, include the required trailers (above). In **product artifacts** — READMEs, UI footers, generated site/HTML, `<meta name="generator">`, CLI banners — do the opposite: strip AI/IDE tool-attribution banners ("Made with Cursor", "Generated by Claude", "Built with Copilot", …). The authoring tool isn't a product feature. Rex flags these as `nit:` / `suggestion:` (advisory, non-blocking).

**Never flag** the framework's own required trailers — `Co-Authored-By: Claude …` on commits and `🤖 Generated with [Claude Code]` on PR bodies (mandated by `CLAUDE.md` § Git). Those are process metadata, not product artifacts.
